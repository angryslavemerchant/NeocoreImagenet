...
def ensure_ram_cache(cfg) -> tuple[Path, Path]:
    """Return (train_blob, val_blob), building anything missing. The jpeg
    cache underneath is bank-accelerated (dataset._ensure_cache pulls the
    2.2 GB tar from Drive via bank.py); the 25 GB blobs themselves rebuild
    locally in minutes and persist via the /workspace symlink."""
